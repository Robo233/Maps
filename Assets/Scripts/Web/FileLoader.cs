...
{
    public static Dictionary<string, byte[]> loadedFiles = new Dictionary<string, byte[]>();

    public static void WriteLoadedFiles() // While the files are being downloaded, theey aren't saved on the disk, but in a dictionary. This method writes them on the disk after the downalod is finsihed. This is because, if the user closes the app during download the files cannot be deleted
    {
    ...
    public static IEnumerator DownloadTextureCoroutine(string url, string path, bool shouldFileBeSaved, Action<Texture2D> OnTextureLoaded)
    {
    ...
    static public IEnumerator DownloadModelCoroutine(string url, string path, string animationType, bool shouldFileBeSaved, Action<GameObject> OnModelLoaded)
    {
        ...
        if(File.Exists(path))
        {
           ...
           // byte[] decryptedModelData = EncryptionHelper.Decrypt(encryptedModelData);
            Model = LoadModel(path, encryptedModelData, animationType, shouldFileBeSaved);
    ...
    static AnimationClip[] AnimationClips;

    static GameObject LoadModel(string path, byte[] modelData, string animationType, bool shouldFileBeSaved = true)
    {
        GameObject Model;
        ...
        byte[] encryptedModelData = modelData;//EncryptionHelper.Encrypt(modelData);
        if(shouldFileBeSaved)
        {
    ...
    public static IEnumerator DownloadAudioCoroutine(string url, string path, bool shouldFileBeSaved, Action<AudioClip> OnAudioLoaded)
    {
        AudioClip audioClip;
        if(File.Exists(path))
        {
    ...
    public static IEnumerator DownloadTextCoroutine(string url, string path, bool shouldFileBeSaved, Action<string> OnTextLoaded, bool shouldCheckIfFileExists = true)
    {
        string text;
    ...
    public static IEnumerator DownloadZipCoroutine(string url, string path, bool shouldFileBeSaved, Action<bool> OnFileDownloaded)
    {
        if (File.Exists(path))
        {
            byte[] encryptedZipBytes = File.ReadAllBytes(path);
            //byte[] decryptedZipBytes = EncryptionHelper.Decrypt(encryptedZipBytes);
            OnFileDownloaded?.Invoke(true);
        }
        else
        {
            using (UnityWebRequest webRequest = UnityWebRequest.Get(url))
            {
                ...
                {
                    Debug.Log(webRequest.error);
                    OnFileDownloaded?.Invoke(false);
                }
                else
                {
                    byte[] downloadedZipBytes = webRequest.downloadHandler.data;
                    byte[] encryptedZipBytes = EncryptionHelper.Encrypt(downloadedZipBytes);

                    if (shouldFileBeSaved)
                    {
                        File.WriteAllBytes(path, encryptedZipBytes);
                    }
                    else if (!loadedFiles.ContainsKey(path))
                    {
                        loadedFiles.Add(path, encryptedZipBytes);
                    }
